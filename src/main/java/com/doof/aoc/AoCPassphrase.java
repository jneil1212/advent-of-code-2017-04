...
public class AoCPassphrase {
    public static void main(String[] args) {
        ...
        // Separate input into single phrases
        String[] split = input.split("\n");

        Set<String> words = new HashSet<>();
        int correctPhrases = 0;

        // Iterate through each of the phrases
        // Add the words in a single phrase to a set
        // Check that the array of words and the set have the same size
        for (String phrase : split) {
            words.clear();
            String[] current = phrase.split(" ");

            for (String word : current) {
                words.add(word);
            }

            if (words.size() == current.length) {
                correctPhrases++;
            }
        }
